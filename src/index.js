    ...
    passenger.find({ passengerUserid: userid })
             .exec((err, passengerResult) => {
               ...
               // the displaying them on the home page, along with the cars they joined
                User.findOne({ _id: userid }).exec((err, userData) => {
  ...
  .post((req, res) => {
    ...
    register.find({ userid: userid })
      .exec((err, data) => {
        ...
        if (isOverlapping) {
          res.render(process.cwd() + '/views/addRegister.html', { message: "Couldn't save data, The Car Pool Overlap with another" })
        } else {

          // else if car pool oppourtity don't overlap all user to create it
          newRegister.save((err, result) => {
            if (!err) {
              res.render(process.cwd() + '/views/addRegister.html', { message: "new Register added successfully" })
            }
            else {
              //err happend while tring to save the register display error message
              res.render(process.cwd() + '/views/addRegister.html', { message: "Couldn't save new Register" })
            }
          })
        }
      })
